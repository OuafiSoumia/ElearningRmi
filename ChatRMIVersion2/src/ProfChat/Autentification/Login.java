...
public class Login extends JFrame {
	...
	public Login() {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				 ...
				 if (!textField.getText().isEmpty() || !passwordField.getText().isEmpty()) {
	                    ...
	                    try {
	                        ps = con.prepareStatement("SELECT login FROM Prof WHERE login=? AND passwd =?");
		...
		btnNewButton.setBounds(228, 165, 85, 21);
		contentPane.add(btnNewButton);
	}
	public void appel() {
		try {
			User user=new User().getProfGuiByLoginAndPasswd(textField.getText(), passwordField.getText());
